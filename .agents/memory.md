...
- **Site (`site/`)**: DONE, **wired to the API and committed** (`56ad560` feat: render published CMS content with ISR + on-demand revalidation; `4da1715` fix: accept POST on revalidate route — branch `site-wired`). Hardcoded `content.ts` is only the fallback when the API is unreachable.
  ...
  - **Whole-site draft preview** (`site/app/preview/page.tsx`, `force-dynamic`, noindex): reads `?pageId=&token=` from searchParams, fetches draft via `getDraftPreview()` (`site/lib/api.ts` → `GET {API}/admin/pages/:id/preview` with `Authorization: Bearer`), renders the real site (layout chrome + section registry) with a floating "Draft preview / View live" badge; friendly messages for missing/invalid/expired links. Admin "Preview site" button opens it in a new tab with the current access token (old in-admin `PreviewOverlay` JSON approximation deleted). Flow: edit (auto-saves draft) → Preview site → Publish.
  - **Per-section preview**: `site/app/preview/section/[template]/page.tsx` renders a single draft section ("Section preview" badge); friendly message if the section no longer exists. Admin: eye icon on each SectionEditor row + "Preview section" button in the expanded footer (`onPreview(section)` → opens new tab). **Chrome rules** via `site/components/layouts/PreviewChrome.tsx` (client, pathname-based, slot pattern in root layout): `/preview` → nav+footer; `/preview/section/hero` → nav only, no footer; `/preview/section/<other>` → neither. Site auth rate limits (api rateLimit.ts): 20/IP + 5/email per 15 min — don't spam logins during testing.
...
- **Admin (`admin/`)**: BUILT (Next.js 15 + Tailwind v4, runs `-p 3001`, consumes live API). All pages implemented, **committed** (`8c9cac6` build + `6a8aba0` fix: toast provider + paginated meta shape), checks green (`typecheck`, `lint`, `build` from `admin/`).
  ...
  - Pages: overview dashboard, pages list + create/delete, page editor (`pages/[id]`: compact section cards → **SectionEditorModal** focused dialog "Editing: <template>" with scrollBody + fixed footer [Preview section / Discard / Save changes]; reorder via `PUT /admin/pages/:pageId/sections/order`, delete, add-section template grid, meta edit, Publish, Versions, whole-site preview in new tab, meta modal). Modal.tsx supports `scrollBody` (max-height + fixed footer).
  ...
  - **Light/dark mode**: class-based theming (`:root` light, `.dark` on `<html>` dark; `@custom-variant dark` in `admin/app/globals.css`). Init inline script (first child of `<body>`) reads `localStorage["tecim.theme"]`, else system `prefers-color-scheme`. `components/theme-provider.tsx` + `components/theme-toggle.tsx` (Sun/Moon) in Shell header + login top-right. Hard-coded `bg-white/[x]`/`bg-black/[x]`/heavy shadows replaced with `bg-overlay`/`bg-overlay-strong`/`bg-scrim`/`shadow-pop`/`shadow-modal` tokens; keep `bg-black/40`/`bg-black/55` for image overlays (text on those = `text-white`). Details captured in `.agents/ui-registry.md` (Admin theme system section).
